...
try {
  ...
  assert(await page.locator('[data-post-card]').count()>0);
  ...
  await page.locator('[data-filter="MDX"]').click();
  assert.equal(await page.locator('[data-post-card]:visible').count(),1);
  await page.locator('[data-filter=""]').click();
  results.push('Tag filtering works.');
  await page.goto(url('blog/writing-lab/'),{waitUntil:'networkidle'});
  ...
  for(const route of ['blog/','resources/','about/','404.html','rss.xml','sitemap-index.xml']){
    ...
    assert(response.ok(),`Route failed: ${route} (${response.status()})`);
  }
