...
- 公共 JSON API 通过 `credentials: omit` 读取，不需要前端访问令牌；数字以接口响应为准，不做本地 `+1`。\n- 新路径第一次读取可能早于 pageview 入库并得到 404。代码会先尝试普通 URL；若 404，则显示「阅读统计同步中」，等待约 12 秒后用官方支持的 `start=1970-01-01` 参数发起一次语义等价的恢复读取，从而避开第一次 404 的缓存键。
- 未配置 →「阅读统计待启用」；真实 0 →「0 次阅读」；首次 404 →「阅读统计同步中」并重试；重试仍 404 →「暂无阅读数据」；错误/超时 →「阅读量暂不可用」。失败不能当作 0。
